...
* ORM = Sequel
* Full capistrano deployment, I mean full: mysql db creation, vhost creation

== Dependencies

None, everything you need is vendored: Sinatra, Sequel, RedCloth.
...
TODO: this is OLD remove: In production, you'll probably want
to run "rake start" to start (and restart) the server.  Change the value of
"port" at the top of the Rakefile to run on a
different port.
...
we are using capistrano, capitate and capinatra for full deployment to configure
a mysql db and set a vhost for apache passenger.

$ cap deploy:setup CAPINATRA=y

set CAPINATRA to anything not nil, it's a hack since capinatra collapses with
capistrano and capitate throwing non set variables exceptions

Then deploy the blog with:
