...
Then /^header "([^\"]*)" is like "([^\"]*)"$/ do |header, value|
  if page.response_headers[header].respond_to? :should
    page.response_headers[header].should =~ /#{value}/
  else
    assert_match(/#{value}/, page.response_headers[header])
  end
end

Then /^header "([^\"]*)" is not like "([^\"]*)"$/ do |header, value|
  if page.response_headers[header].respond_to? :should_not
    page.response_headers[header].should_not =~ /#{value}/
  else
    assert_no_match(/#{value}/, page.response_headers[header])
  end
end

# TODO: Add "cookie exists", "cookie doesn't exisits" etc
Then /^cookie "([^\"]*)" should be like "([^\"]*)"$/ do |cookie, value|
  ...
  if cookie_value.respond_to? :should
    ...
    assert_match(/#{value}/, cookie_value)
  end 
end

Then /^cookie "([^\"]*)" should not be like "([^\"]*)"$/ do |cookie, value|
  cookie_value = Capybara.current_session.driver.request.cookies.[](cookie)
  if cookie_value.respond_to? :should_not
    cookie_value.should_not =~ /#{value}/
  else
    assert_no_match(/#{value}/, cookie_value)
  end 
end

# TODO: puts page.status_code